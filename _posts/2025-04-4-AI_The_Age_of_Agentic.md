...
Agentic AI requires new approaches and architecture. Message or event-driven architectures don't really fit well. In addition, new ways of connecting agents to data are emerging. One such standard from Anthropic, called Model Context Protocol [MCP](https://www.anthropic.com/news/model-context-protocol) has already started to gain traction. Agentic requires more focus on a conversation-driven approach, where the clear boundaries and definitions provided by a message or event-driven architecture, simply do not exist.
...
Finally, the LLM is a generalist and will require specialist tools to complete tasks. Those tools could be other LLMs/SLMs, Inference, Retrieval-Augemented-Generation, even sub-agents (any external API).
...
Choosing the right technology to build upon is crucial. Orchestration is important, but only orchestration that can support long-running (infinite) conversations, while providing state management and durability. Temporal becomes a natural fit for orchestrating Human-to-AI conversation.
...
There are two aspects to Agentic: conversation and tools. Using Temporal, both can be modelled as Workflows.
