...
## Replay
Replay in Temporal is a fundamental part of ensuring durable and reliable workflow executions. When a workflow executes, each action it makes is recorded in an Event History. If a workflow fails or is reset, Temporal uses this Event History to replay the workflow execution from the beginning to the point of reset or failure and then resuming normal execution. Replay ensures that workflows are not only durable, but can be confidently used for long-running and mission-critical applications.

Replay is built-in to Temporal. Unless looking carefully, users would not even notice a replay occurring. They would just observe, workflows continue, after failure condition blocking forward progress is resolved.
